...
class AccountController extends FrontendController
{
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()){
                if ($model->editAccount()) {
                    Yii::$app->session->setFlash('success', Yii::t('app', 'Изменения приняты.'));
                } else {
    ...
    public function actionRequestPasswordReset()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                Yii::$app->session->setFlash('success', Yii::t('app', 'Проверьте свою почту и следуйте инструкциям.'));

                return $this->goHome();
            } else {
                Yii::$app->session->setFlash('error', Yii::t('app', 'Простите, нам не удалось сбросить пароль для указанного e-mail адреса.'));
            }
        }
