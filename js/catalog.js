...
(function($){
    $(function(){
        ...
        function floating_cols() {
            let w_p = $(window).scrollTop();
            let w_w = $(window).width();

            $('.float-col').each(function () {
                let $this = $(this);
                let floating = $this.data('floating');

                let tans = w_p*floating;

                if (w_w > 960) {
                    $this.css('-webkit-transform','translate(0,' + -tans + 'px)');
                } else {
        ...
        floating_cols();
        function floating_cols_items() {
            $('.jq_animate').each(function(){
                let scroll_pos = window.pageYOffset;
                if ($(this).offset().top - 100 < scroll_pos + $(window).height()) {
                    $(this).addClass('jq_shown').removeClass('jq_top_hide');
                }
                // if ($(this).offset().top + 50< scroll_pos) {
                //     $(this).addClass('jq_top_hide').removeClass('jq_shown');
                // }
                if ($(this).offset().top > scroll_pos + $(window).height()) {
                    $(this).removeClass('jq_shown jq_top_hide');
                }
        ...
        $('.prev-c-item').click(function () {
            s_c_change (-1);
        });
        $('.next-c-item').click(function () {
            s_c_change (1);
        });

        function s_c_change (next_i) {
            let index = $('.sc-items-container').find('.active_item').index();
            let total = $('.sc-items-container').find('.item').length;

            let new_index = index + next_i;


            if (total <= new_index || 0 > new_index) {
                return false;
            }

            let $next_item = $('.sc-items-container').find('.item').eq(new_index);
            let new_image = $next_item.data('image');
            console.log(new_image);

            $('.fader').addClass('active');

            $('.title-block').addClass('fade_out');
            $('.works-nav').addClass('fade_out');

            setTimeout(function () {
                $('.fader').addClass('fade-out');
                $('.active_item').removeClass('active_item');
                $('.item-bg').addClass('ff_left');
                $('.bg-image').css({
                    'background-image' : 'url(' + new_image + ')'
                });
            }, 1200);
            setTimeout(function () {
                $('.item-bg').addClass('active').removeClass('ff_left');
            }, 1600);
            setTimeout(function () {
                $('.fader').removeClass('fade-out active');
                $next_item.addClass('active_item');
                $('.item-bg').removeClass('active');
                $('.title-block').removeClass('fade_out');
                $('.works-nav').removeClass('fade_out');
        ...
        if ($('.single-blocks-container').length) {
            $('#footer').addClass('no-footer');
        }

        $('.p-ajax-link').click(function (event) {
            let $image_holder = $('#image_ani_holder');
            let $image_block = $(this).parents('.item').find('.image');
            let src = $image_block.attr('src');
            // console.log(src);
            $image_holder.find('.t-image').attr('src', src);

            let w_top = $(window).scrollTop();

            let img_top = $image_block.offset().top;
            let img_left = $image_block.offset().left;
            let img_h = $image_block.height();
            let img_ww = $image_block.width();
            ...
            $.ajax({
                ...
                success: function(data){
                    let $data = $(data).find('#ajax_content');
                    $('.product-pop-content').html($data);
                    window.history.pushState('item', 'item', url);
                },
                complete: function() {
                    $image_block.addClass('hide-out');
                    $('.fader').addClass('active');
                    ...
                    setTimeout(function () {
                        ...
                        $image_block.removeClass('hide-out');
                        $('.fader').removeClass('active');

                    }, 2400);

                    let viewer = new Viewer(document.getElementById('viewer_image'), {
                        inline: false,
                        navbar: false,
                        toolbar: false,
                        viewed() {
                    ...
                    $(document).on('click', '.fullscreen-btn' ,function (event) {
                        viewer.show();
                    });


                    $('.p-pop-close').click(function (event) {
                        event.preventDefault();
                        $('.product-popup').removeClass('active');
                        $('#content').removeClass('no-scroll');
                        $('#footer').removeClass('no-footer');
                        window.history.pushState('catalog', 'catalog', $(this).attr('href'));
                    });
        ...
        $(document).on('click', '.interior-btn' ,function (event) {
            let $image_block = $('.int-image-cont');
            let w_top = $(window).scrollTop();

            let img_top = $image_block.offset().top;
            let img_left = $image_block.offset().left;
            let img_h = $image_block.height();
            let img_w = $image_block.width();
            ...
            let new_img_w = 200;
            let new_img_h = new_img_w/img_w*img_h;



            $image_block.animate({
                top: w_h/2 - new_img_h/2 - w_h*0.1 + 'px',
                left: w_w/2 - 100 + 'px',
                width: new_img_w + 'px',
                height: new_img_h + 'px',
            ...
            setTimeout(function () {
                $('.interior-popup').addClass('active');
            }, 600);
            setTimeout(function () {
                $image_block.addClass('on_the_wall');
            }, 1200);


            $(document).on('click', '.interior-popup' ,function (event) {
                $image_block.removeClass('on_the_wall');
                ...
                setTimeout(function () {
                    $('.interior-popup').removeClass('active');
                }, 600);

                setTimeout(function () {
                    $image_block.finish();
                    $image_block.removeAttr("style");
                }, 1250);
